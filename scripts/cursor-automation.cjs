...
function buildTriageComment(classification, { issueCommentWatermark } = {}) {
...
function decideIssueCommentRoute({
  ...
  if (trustedAssociation && mentionsIssueBot(body, botLogins)) {
    return {
      kind: 'issue_followup',
...
async function prepareIssueContext({
  ...
  if (needsInfoReply && !manual) {
    const processed = extractProcessedIssueFollowupIds(commentList, botLogins);
    const watermark = extractIssueTriageWatermark(commentList, botLogins);
    if (processed.has(triggerId) || commentIdAtOrBefore(triggerId, watermark)) {
      setOutput(core, 'should_run', false);
  ...
  setOutput(
    core,
    'latest_comment_id',
    commentList.length ? String(commentList[commentList.length - 1].id || '') : '',
  );
...
async function applyClassification({
  ...
  try {
    await github.rest.issues.createComment({
      ...
      repo,
      issue_number: issue.number,
      body: buildTriageComment(classification, { issueCommentWatermark }),
    });
  } catch (error) {
    try {
      await github.rest.issues.update({
...
async function restoreCleanPullRequestAfterNoChange({
  ...
  if (before.pending.length) return false;
...
async function prepareIssueFollowupContext({
  ...
  const pending = findPendingIssueFollowups({
    ...
    issueAuthorLogin: issue.user?.login,
    pull,
    triggerCommentId: pull ? undefined : triggerCommentId,
    botLogins,
  });
  const startOfDay = new Date(nowMs);
  startOfDay.setUTCHours(0, 0, 0, 0);
