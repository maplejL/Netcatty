...
|---|---|---:|---|---|---|
| Netcatty 当前分支 | 固定偏移并发读写；优先独立通道，失败时尝试共享通道 | 上传 32 × 32 KiB；下载 64 × 32 KiB（[配置](https://github.com/binaricat/Netcatty/blob/e1793e382bf022f792a74cfca4d7de95c92bd5bf/electron/bridges/transferLimits.cjs#L3-L23)） | 只把连续完成区间记作断点；暂停等待在途请求；取消时关闭临时通道 | 面板上传：普通文件先暂存再替换，符号链接原位写入，并有备份恢复；传输中心仍使用另一套 `.part` 发布流程 | 传输中心的本地可续传上传逐块校验 SHA-256；面板上传检查远端大小并在替换前恢复原权限；两者尚未统一 |
