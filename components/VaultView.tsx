...
interface VaultViewProps {
  ...
  onOpenBatchSftp?: (hosts: Host[]) => void;
...
const VaultViewInner: React.FC<VaultViewProps> = ({
  ...
  const openBatchSftpForSelection = useCallback(() => {
    if (selectedHostIds.size === 0 || !onOpenBatchSftp) return;
    const targets = hosts.filter((h) => selectedHostIds.has(h.id));
    onOpenBatchSftp(targets);
  }, [selectedHostIds, hosts, onOpenBatchSftp]);
