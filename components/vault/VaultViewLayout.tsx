        ...
        >
          ...
          <div className={cn(
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <RippleButton
                  variant={currentSection === "agentHistory" ? "secondary" : "ghost"}
                  className={cn(
                    "w-full h-10",
                    sidebarCollapsed ? "justify-center p-0" : "justify-start gap-3",
                    currentSection === "agentHistory" &&
                    "bg-foreground/10 text-foreground hover:bg-foreground/15 border-border/40",
                  )}
                  onClick={() => setCurrentSection("agentHistory")}
                >
                  <Bot size={16} className="flex-shrink-0" />
                  {!sidebarCollapsed && t("vault.nav.agentHistory")}
                </RippleButton>
              </TooltipTrigger>
              {sidebarCollapsed && <TooltipContent side="right">{t("vault.nav.agentHistory")}</TooltipContent>}
            </Tooltip>
          </div>
      ...
      <div className="flex min-h-0 min-w-0 flex-1 py-0 pr-2 pb-2 pl-0" data-section="vault-stage">
        ...
        {isMultiSelectMode && isHostsSectionActive && (
          <div className="px-4 py-1.5 bg-background border-b border-border/40 flex items-center gap-2">
            ...
            {openBatchSftpForSelection && (
              <Button
                variant="outline"
                size="sm"
                className="h-7 px-2 text-xs"
                disabled={selectedHostIds.size === 0}
                onClick={openBatchSftpForSelection}
              >
                <Upload size={12} className="mr-1" />
                {t("vault.hosts.batchSftp", { count: selectedHostIds.size })}
              </Button>
            )}
            <Button
              variant="destructive"
        ...
        {/* Agent / coding CLI terminal history */}
        {currentSection === "agentHistory" && (
          <LazyLoadBoundary name="Agent terminal history" resetKey="agent-history">
            <Suspense fallback={<VaultSectionLoading />}>
              <LazyCodingCliTerminalHistoryPanel
                variant="page"
                entries={codingCliHistoryEntries ?? []}
                onOpenEntry={onOpenCodingCliHistoryEntry}
                onRemoveEntry={onRemoveCodingCliHistoryEntry}
                onTogglePin={onToggleCodingCliHistoryPin}
                onClearAll={onClearCodingCliHistory}
                onScanOpenTerminals={onScanOpenCodingCliTerminals}
                autoScanOnOpen
              />
            </Suspense>
          </LazyLoadBoundary>
