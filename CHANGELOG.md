...
? fork ?????????? `feature/ops_ai_enhancements`?**???? `main`**??

### ? fork ?? / ??

#### ??????
- ??????????? `Ctrl+V`?SFTP ?????? `Ctrl+Shift+V`?
- ??????????? Chromium ?????? / ?????
- ?????????????????????????????
- ?? Bash `Esc + .` ????????????????
...
- ????? CLI ????? AI ?????????????????
- ??????????????????????????????
- ?? Netcatty CLI `exec` / `job-start` ? `sessions is not defined` RPC ???
- ?? AI ??????????????????
...
### ??
- ??????? NSIS??`npm run pack:win-x64:release`
- ????NSIS + portable + zip??`npm run pack:win-x64:release:full`
- ??????????`npm run release:win-x64`
- ???? asar?`npm run pack:asar`
